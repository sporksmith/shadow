...
static void _inject_trampoline(struct ParsedElf* parsedElf, const char* vdsoFnName,
    ...
    // TODO: another trampoline strategy we could implement if neither of these work
    // is to inject `ud2; ret`, or even just `ud2`. `ud2` is only 2 bytes and raises SIGILL;
    // we could get control in the SIGILL signal handler and figure out which patched
    // function we're trying to execute by inspecting the instruction pointer in the siginfo_t.
